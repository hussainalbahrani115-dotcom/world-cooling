...
- [x] **Phase 6 — تطبيق الموبايل (Flutter)**: تطبيق واحد لـ Android + iOS يعكس مسار
      ...
      → بوابة دفع → نتيجة التشخيص وقطع الغيار).
...
├── mobile/         # Flutter (Android + iOS) — تطبيق المستخدم النهائي (Phase 6)
└── docker-compose.yml
```

ربط بوابة الدفع الحقيقية (HyperPay/Moyasar، Phase 4) وحدها مؤجَّلة عمداً بقرار صاحب المنتج.
...
## تطبيق الموبايل (mobile/ — Phase 6)

مشروع Flutter قياسي (نسخة واحدة لـ Android + iOS، مُنشأ عبر `flutter create`) يعكس مسار
المستخدم من قسم الرؤية في المواصفات:

1. **`ApplianceSelectionScreen`** — اختيار نوع الجهاز، مجمّعة حسب التصنيف (تكييف/ثلاجات/أفران)
   من `GET /catalog/appliances`.
2. **`SymptomSelectionScreen`** — اختيار عرض أولي من `GET /catalog/appliances/:id/symptoms`،
   أو تخطّيه والبدء مباشرة.
3. **`DiagnosisFlowScreen`** — شاشة واحدة تتبدّل حسب نوع استجابة الجلسة من محرك التشخيص
   (Phase 3): أسئلة متسلسلة مع شريط "نسبة الثقة"، ثم بوابة دفع (زر يستدعي
   `simulate-payment` المؤقت مع ملاحظة "وضع تجريبي" ظاهرة للمستخدم — بصراحة، دون إيهامه
   بدفع حقيقي لم يُبنَ بعد)، ثم نتيجة التشخيص الكاملة مع قطع الغيار وأزرار شراء تفتح رابط
   المتجر عبر `url_launcher`.
